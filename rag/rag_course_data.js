(() => {
  ...
  const wikiP = ["### YOUR CODE HERE ###","import wikipedia","wikipedia.????(\"my-rag-project (test@example.com)\")","reader = ????()","# city_names에 담긴 도시 이름들을 입력으로 받아 Wikipedia 문서를 Document 목록으로 읽어옵니다.","# 전달한 도시 이름을 그대로 검색하기 위해, 자동 후보 보정은 사용하지 않습니다.","documents = reader.????(city_names, auto_suggest=False)","","index = VectorStoreIndex.????(documents)","query_engine = index.????()","response = query_engine.????(\"What's the arts and culture scene in Berlin?\")"];
  const wikiA = fill(wikiP,{2:'wikipedia.set_user_agent("my-rag-project (test@example.com)")',3:"reader = WikipediaReader()",6:"documents = reader.load_data(city_names, auto_suggest=False)",8:"index = VectorStoreIndex.from_documents(documents)",9:"query_engine = index.as_query_engine()",10:'response = query_engine.query("What\'s the arts and culture scene in Berlin?")'});
  ...
  const evalP = ["### YOUR CODE HERE ###","def parse_response(response):","    try:","        response = response.????()","        model_resp = json.????(response)","        answer = -1","        if \"accuracy\" in model_resp and (","            model_resp[\"accuracy\"] is True or","            (isinstance(model_resp[\"accuracy\"], str) and model_resp[\"accuracy\"].lower() == \"true\")):","            answer = ????","        else:","            raise ValueError(f\"Could not parse answer from response: {model_resp}\")","        return answer","    except:","        return ????","","def CRAG_evaluation(question, ground_truth, prediction):","  context_template = f\"Question: {question}\\nGround Truth: {ground_truth}\\nPrediction: {prediction}\"","  evaluation_result = generate_answer(user_prompt=context_template, system_prompt=????)","  eval_res = ????(evaluation_result)","  return eval_res"];
  const evalA = fill(evalP,{3:"        response = response.lower()",4:"        model_resp = json.loads(response)",9:"            answer = 1",14:"        return -1",18:"  evaluation_result = generate_answer(user_prompt=context_template, system_prompt=INSTRUCTIONS)",19:"  eval_res = parse_response(evaluation_result)"});

  const scoreP = ["### YOUR CODE HERE ###","prediction_lowercase = rag.inference(question, web_search_results, 5)[????].lower()","if prediction_lowercase == ground_truth_lowercase:","    n_correct_exact += 1","elif \"i don't know\" in prediction_lowercase:","    n_miss += 1","else:","    acceptable = ????(question, ground_truth_lowercase, prediction_lowercase)","    if acceptable == 1:","        n_correct += 1","n_hallucinate = len(finance_test_dataset_ids) - ???? - ???? - ????","CRAG_score = ???? + 0.5*???? - ????"];
  const scoreA = fill(scoreP,{1:"prediction_lowercase = rag.inference(question, web_search_results, 5)['answer'].lower()",7:"    acceptable = CRAG_evaluation(question, ground_truth_lowercase, prediction_lowercase)",10:"n_hallucinate = len(finance_test_dataset_ids) - n_correct_exact - n_correct - n_miss",11:"CRAG_score = n_correct_exact + 0.5*n_correct - n_hallucinate"});

  const combinedP = ["### YOUR CODE HERE ###","class RAG:","    def __init__(self, server=None):","        self.retriever = LlamaIndexRetriever()","        self.kg_query_engine = KGQueryEngine(server=server)","        self.reader = Reader()","    def retrieve(self, query, search_results, topk):","        retrieved_results = self.retriever.????(query, search_results, topk)","        kg_results = self.kg_query_engine.????(query)","        combined_results = [????]","        combined_results.????(retrieved_results)","        return combined_results","    def inference(self, query, search_results, topk):","        retrieved_results = self.????(query, search_results, topk)","        answer = self.????(query, retrieved_results)","        return {\"retrieved_results\": retrieved_results, \"answer\": answer}"];
  const combinedA = fill(combinedP,{7:"        retrieved_results = self.retriever.retrieve(query, search_results, topk)",8:"        kg_results = self.kg_query_engine.query(query)",9:"        combined_results = [kg_results]",10:"        combined_results.extend(retrieved_results)",13:"        retrieved_results = self.retrieve(query, search_results, topk)",14:"        answer = self.generate_response(query, retrieved_results)"});

  const mcpClientP = ["### YOUR CODE HERE ###","external_mcp_server = \"http://10.2.0.165:8081/sse\" #change to correct uri","mcp_client = ????(external_mcp_server)","mcp_tool = ????(client=mcp_client)","tools = await mcp_tool.????()","for tool in tools:","    print(tool.metadata.name)","resources = await mcp_tool.????()","for resource in resources:","    print(resource.uri)"];
  const mcpClientA = fill(mcpClientP,{2:"mcp_client = BasicMCPClient(external_mcp_server)",3:"mcp_tool = McpToolSpec(client=mcp_client)",4:"tools = await mcp_tool.to_tool_list_async()",7:"resources = await mcp_tool.fetch_resources()"});

  const mcpEngineP = ["### YOUR CODE HERE ###","class KGQueryEngineWithMCP:","    def __init__(self, mcp_tool_spec: McpToolSpec, model: str, llm = None):","        self.llm = llm or OpenAI(model=model, temperature=0)","        self.mcp_tool_spec = mcp_tool_spec","        self.agent = None","        self.agent_context = None","    async def init_agent(self):","        tools = await self.mcp_tool_spec.????()","        self.agent = ????(","            name=\"Agent\", tools=tools, llm=self.llm, system_prompt=SYSTEM_PROMPT)","        self.agent_context = ????(self.agent)","    async def query(self, question: str, verbose: bool = False) -> str:","        if self.agent is None or self.agent_context is None:","            raise RuntimeError(\"Agent not initialized. Call `await init_agent()` first.\")","        handler = self.agent.????(question, ctx=self.agent_context)","        async for event in handler.????():","            if verbose and type(event) == ToolCall:","                print(event.tool_name)","        response = await ????","        return str(response)"];
  const mcpEngineA = fill(mcpEngineP,{8:"        tools = await self.mcp_tool_spec.to_tool_list_async()",9:"        self.agent = FunctionAgent(",11:"        self.agent_context = Context(self.agent)",15:"        handler = self.agent.run(question, ctx=self.agent_context)",16:"        async for event in handler.stream_events():",19:"        response = await handler"});

  const mcpRagP = ["### YOUR CODE HERE ###","class RAGwithMCP:","    def __init__(self, mcp_tool):","        self.mcp_application = ????(mcp_tool, model='gpt-4o-mini')","        self.reader = ????()","    async def retrieve(self, query: str, query_time: str, search_results: list, topk: int):","        await self.mcp_application.????()","        full_query = f\"Query: {query}\\nQuery time: {query_time}\"","        mcp_result = await self.mcp_application.????(full_query, verbose=False)","        return mcp_result","    def generate_response(self, query: str, query_time: str, retrieved_results: str):","        answer = self.reader.????(query, query_time, retrieved_results)","        return answer","    async def inference(self, query: str, search_results: list, query_time: str, topk: int):","        retrieved_results = await self.????(query, query_time, search_results, topk)","        answer = self.????(query, query_time, retrieved_results)","        return {\"retrieved_results\": retrieved_results, \"answer\": answer}"];
  const mcpRagA = fill(mcpRagP,{3:"        self.mcp_application = KGQueryEngineWithMCP(mcp_tool, model='gpt-4o-mini')",4:"        self.reader = Reader()",6:"        await self.mcp_application.init_agent()",8:"        mcp_result = await self.mcp_application.query(full_query, verbose=False)",11:"        answer = self.reader.generate_response(query, query_time, retrieved_results)",14:"        retrieved_results = await self.retrieve(query, query_time, search_results, topk)",15:"        answer = self.generate_response(query, query_time, retrieved_results)"});

  window.LLM_COURSE = {
    subject:"2. RAG", sample_mode:false,
    cells:{
      "rag1-q1":{source:'SimpleDirectoryReader("data").load_data()'},"rag1-q2":{source:"VectorStoreIndex.from_documents(documents)"},"rag1-q3":{source:"index.as_query_engine()"},"rag1-q4":{source:"response = query_engine.query(question)"},
      ...
      "mcp-q1":{source:"response.lower()"},"mcp-q2":{source:"json.loads(response)"},"mcp-q3":{source:"answer = 1"},"mcp-q4":{source:"return -1"},"mcp-q5":{source:"system_prompt=INSTRUCTIONS"},"mcp-q6":{source:"parse_response(evaluation_result)"},
      "mcp-q7":{source:"rag.inference(question, web_search_results, 5)['answer']"},"mcp-q8":{source:"CRAG_evaluation(question, ground_truth_lowercase, prediction_lowercase)"},"mcp-q9":{source:"n_correct_exact + 0.5*n_correct - n_hallucinate"},
      "mcp-q10":{source:"self.retriever.retrieve(query, search_results, topk)"},"mcp-q11":{source:"combined_results = [kg_results]"},"mcp-q12":{source:"combined_results.extend(retrieved_results)"},
      "mcp-q13":{source:"BasicMCPClient(external_mcp_server)"},"mcp-q14":{source:"McpToolSpec(client=mcp_client)"},"mcp-q15":{source:"mcp_tool.to_tool_list_async()"},"mcp-q16":{source:"mcp_tool.fetch_resources()"},
      "mcp-q17":{source:"self.mcp_tool_spec.to_tool_list_async()"},"mcp-q18":{source:"FunctionAgent("},"mcp-q19":{source:"Context(self.agent)"},"mcp-q20":{source:"self.agent.run(question, ctx=self.agent_context)"},"mcp-q21":{source:"handler.stream_events()"},"mcp-q22":{source:"await handler"},
      "mcp-q23":{source:"KGQueryEngineWithMCP(mcp_tool, model='gpt-4o-mini')"},"mcp-q24":{source:"self.mcp_application.init_agent()"},"mcp-q25":{source:"self.mcp_application.query(full_query, verbose=False)"},"mcp-q26":{source:"self.reader.generate_response(query, query_time, retrieved_results)"},"mcp-q27":{source:"await self.retrieve(query, query_time, search_results, topk)"}
    },
    chapters:[{
      ...
      theory_guide:[
        ...
        {title:"5. Custom Query Engine 구성",concept:"Retriever, Response Synthesizer, LLM, PromptTemplate을 목적에 맞게 주입하여 자신만의 Query Engine을 구성합니다.",flow:"부품 생성 → OurCustomQueryEngine(...) → query(question) → response",code_signal:"retriever=, response_synthesizer=, llm=, qa_prompt= 네 keyword 인자를 연결합니다.",exam_clue:"생성식 전체 또는 괄호 안 네 인자를 빈칸으로 낼 수 있습니다."}
      ],
      ...
      mcq:[
        ...
        {id:"rag1-m-query",source_question_id:"rag1-q4",topic:"[기출 대비] QueryEngine 실행",prompt:"질문 변수 question을 RAG QueryEngine에 넣고 최종 답변을 response에 저장하는 한 줄은?",answer_index:1,explanation:"query_engine.query(question)는 retrieval과 response synthesis를 수행해 최종 응답을 반환합니다.",choices:[{text:"response = retriever.retrieve(question)",why:"검색 Node 목록이지 최종 답변이 아닙니다."},{text:"response = query_engine.query(question)",why:"정답입니다. QueryEngine의 최종 질의 호출입니다."},{text:"response = index.as_query_engine(question)",why:"Engine 생성 단계이며 question을 받지 않습니다."},{text:"response = VectorStoreIndex.from_documents(question)",why:"문서로 index를 만드는 단계입니다."},{text:"response = query_engine.retrieve(question)",why:"기본 QueryEngine의 실행 API가 아닙니다."}]},
      ...
      full_code_cells:[cell(7,answerP,answerA,[6,7,9]),cell(25,wikiP,wikiA,[2,3,6,8,9,10]),cell(38,scratchP,scratchA,[4,7,13,14]),cell(45,configP,configA,[1,2,3,4,6,7]),cell(56,custom2P,custom2A,[7,8,9,10,16]),cell(64,refineP,refineA,[3,4,5,9,12,14,15])],
      subjective:[
        ...
        S("rag2-q4","wikipedia.set_user_agent","Wikipedia 설정","Wikipedia 요청의 user agent를 지정하는 메서드까지 쓰세요."),S("rag2-q5","WikipediaReader()","Reader","Wikipedia 문서 reader를 생성하세요."),S("rag2-q6","reader.load_data(city_names, auto_suggest=False)","문서 로딩","city_names의 도시 이름을 Document 목록으로 읽고, 입력 이름을 임의 보정하지 않도록 auto_suggest를 끈 호출을 쓰세요."),
      ...
      id:"rag-06",number:"06",title:"RAG Evaluation & MCP",file:"4_RAG_framework_evaluation_with_MCP.ipynb",
      capability:"CRAG 규칙으로 RAG 답을 채점하고 MCP client의 tool 목록을 FunctionAgent에 연결해 query-time 인식 KG RAG를 비동기로 구현할 수 있다.",
      summary:"RAG 예측을 exact·acceptable·miss·hallucination으로 분류해 점수를 계산하고, MCP server의 KG tools를 agent가 선택·호출하도록 구성해 정형 query generator를 대체합니다.",
      notebook_goal:"RAG를 CRAG metric으로 평가하고 MCP tool calling 기반 KG retrieval로 업그레이드한다.",
      key_points:[
        {title:"LLM Judge",purpose:"질문·정답 후보·예측을 평가 prompt에 넣고 Accuracy JSON을 1/-1로 파싱합니다.",code:"generate_answer(... INSTRUCTIONS) → parse_response",flow:"prediction+ground truth → JSON judgement",watch:"모르겠다는 답은 규칙상 miss이며 correct가 아닙니다."},
        {title:"CRAG Score",purpose:"exact와 의미상 correct에는 보상, hallucination에는 벌점을 줍니다.",code:"exact + 0.5*correct - hallucinate",flow:"answer classes → scalar score",watch:"miss는 직접 벌점은 없지만 점수를 얻지 못합니다."},
        {title:"Web+KG 근거",purpose:"KG 결과와 Web Top-k 결과를 하나의 reference 목록으로 결합합니다.",code:"[kg_results].extend(retrieved_results)",flow:"KG + Web → Reader",watch:"이전 Task 2의 선택 routing과 달리 둘을 항상 합칩니다."},
        {title:"MCP Client·Tool",purpose:"MCP server에서 agent가 호출 가능한 도구와 resource를 동적으로 가져옵니다.",code:"BasicMCPClient → McpToolSpec → to_tool_list_async",flow:"server → tool metadata → FunctionAgent",watch:"SSE endpoint URI와 await가 필요합니다."},
        {title:"MCP RAG",purpose:"query와 query_time을 agent에 보내 tool 결과를 얻고 Reader로 최종 답을 생성합니다.",code:"init_agent → await query → Reader.generate_response",flow:"question+time → tool calls → evidence → answer",watch:"agent는 먼저 비동기 초기화해야 합니다."}
      ],
      theory_guide:[
        {title:"1. CRAG 평가",concept:"CRAG 평가는 exact match, 의미상 correct, miss, hallucination을 다르게 취급합니다.",flow:"exact=+1, acceptable=+0.5, hallucination=-1",code_signal:"두 continue 뒤 LLM judge가 실행됩니다.",exam_clue:"hallucination 수는 전체에서 세 분류 수를 뺍니다."},
        {title:"2. MCP",concept:"MCP는 model이 외부 server가 공개한 tool과 resource를 표준 방식으로 발견·호출하게 합니다.",flow:"client → tool spec → tool list → agent",code_signal:"to_tool_list_async 결과가 FunctionAgent tools로 들어갑니다.",exam_clue:"네트워크·tool 호출은 async/await 흐름입니다."},
        {title:"3. FunctionAgent",concept:"FunctionAgent는 질문과 tool 설명을 보고 필요한 도구를 선택하고 결과를 이어서 처리합니다.",flow:"run → stream tool events → await final response",code_signal:"Context가 여러 단계의 agent 상태를 보존합니다.",exam_clue:"init_agent 전 query를 막는 guard가 있습니다."},
        {title:"4. Query time",concept:"동적 질문은 상대 시간 표현을 실제 질의 시점에 고정해야 정확한 KG 결과를 얻습니다.",flow:"query + literal query_time → MCP tool call",code_signal:"full_query에 Query time을 명시합니다.",exam_clue:"Reader도 question, query_time, references 세 값을 받습니다."}
      ],
      full_code_cells:[cell(17,evalP,evalA,[3,4,9,14,18,19]),cell(25,scoreP,scoreA,[1,7,10,11]),cell(22,combinedP,combinedA,[7,8,9,10,13,14]),cell(42,mcpClientP,mcpClientA,[2,3,4,7]),cell(46,mcpEngineP,mcpEngineA,[8,9,11,15,16,19]),cell(55,mcpRagP,mcpRagA,[3,4,6,8,11,14,15])],
      subjective:[
        S("mcp-q1","response.lower()","평가 파싱","평가 JSON을 대소문자와 무관하게 처리하세요."),S("mcp-q2","json.loads(response)","평가 JSON","평가 문자열을 객체로 파싱하세요."),S("mcp-q3","answer = 1","Correct","Accuracy가 true일 때 반환값을 지정하세요."),S("mcp-q4","return -1","평가 실패","파싱 실패 또는 불일치 시 값을 반환하세요."),S("mcp-q5","system_prompt=INSTRUCTIONS","Judge prompt","평가 규칙을 system prompt로 전달하는 인자를 쓰세요."),S("mcp-q6","parse_response(evaluation_result)","Judge 결과","평가 LLM 출력을 수치로 파싱하세요."),
        S("mcp-q7","rag.inference(question, web_search_results, 5)['answer']","RAG 예측","평가할 RAG answer를 가져오는 표현을 쓰세요."),S("mcp-q8","CRAG_evaluation(question, ground_truth_lowercase, prediction_lowercase)","의미 평가","exact도 miss도 아닌 예측을 judge하는 호출을 쓰세요."),S("mcp-q9","n_correct_exact + 0.5*n_correct - n_hallucinate","CRAG 점수","CRAG score 우변을 쓰세요."),
        S("mcp-q10","self.retriever.retrieve(query, search_results, topk)","Web 근거","Top-k Web 결과를 검색하세요."),S("mcp-q11","combined_results = [kg_results]","KG 목록","KG 결과로 reference 목록을 시작하세요."),S("mcp-q12","combined_results.extend(retrieved_results)","근거 결합","Web 결과를 KG 목록 뒤에 추가하세요."),
        S("mcp-q13","BasicMCPClient(external_mcp_server)","MCP Client","SSE server에 연결할 MCP client를 생성하세요."),S("mcp-q14","McpToolSpec(client=mcp_client)","Tool Spec","client를 LlamaIndex tool spec으로 감싸세요."),S("mcp-q15","mcp_tool.to_tool_list_async()","Tool discovery","MCP 도구 목록을 비동기로 가져오는 호출을 쓰세요."),S("mcp-q16","mcp_tool.fetch_resources()","Resource discovery","MCP resource를 가져오는 호출을 쓰세요."),
        S("mcp-q17","self.mcp_tool_spec.to_tool_list_async()","Agent tools","agent 초기화에서 tools를 가져오는 호출을 쓰세요."),S("mcp-q18","FunctionAgent(","Agent","tool-calling agent 생성자 시작을 쓰세요."),S("mcp-q19","Context(self.agent)","Agent Context","agent workflow context를 생성하세요."),S("mcp-q20","self.agent.run(question, ctx=self.agent_context)","Agent 실행","질문과 context로 agent 실행 handler를 만드세요."),S("mcp-q21","handler.stream_events()","Tool event","tool 호출 event stream을 순회할 호출을 쓰세요."),S("mcp-q22","await handler","최종 응답","agent handler의 최종 응답을 기다리세요."),
        S("mcp-q23","KGQueryEngineWithMCP(mcp_tool, model='gpt-4o-mini')","MCP KG","MCP 기반 KG application을 생성하세요."),S("mcp-q24","self.mcp_application.init_agent()","Agent 초기화","retrieve 시작 시 agent를 초기화하는 호출을 쓰세요."),S("mcp-q25","self.mcp_application.query(full_query, verbose=False)","MCP 검색","query와 time이 합쳐진 질문을 실행하세요."),S("mcp-q26","self.reader.generate_response(query, query_time, retrieved_results)","시간 인식 Reader","질문·시각·근거로 최종 답을 생성하세요."),S("mcp-q27","await self.retrieve(query, query_time, search_results, topk)","Async inference","비동기 MCP retrieval 결과를 기다리세요.")
      ],
      mcq:[
        {id:"mcp-m1",source_question_id:"mcp-q9",topic:"CRAG Score",prompt:"CRAG score의 올바른 계산식은?",answer_index:1,explanation:"exact는 1, acceptable은 0.5, hallucination은 -1입니다.",choices:[{text:"exact+correct+miss",why:"miss에 점수를 주고 hallucination 벌점이 없습니다."},{text:"exact+0.5*correct-hallucinate",why:"정답입니다."},{text:"0.5*exact-correct",why:"가중치가 뒤바뀝니다."},{text:"exact-miss",why:"의미상 correct와 hallucination을 무시합니다."},{text:"correct/hallucinate",why:"평가 정의가 아닙니다."}]},
        {id:"mcp-m2",source_question_id:"mcp-q12",topic:"근거 결합",prompt:"KG 결과 목록에 Web 검색 결과 여러 개를 이어 붙이는 코드는?",answer_index:3,explanation:"extend는 list 요소들을 개별 reference로 추가합니다.",choices:[{text:"combined_results.append([retrieved_results])",why:"중첩 목록이 됩니다."},{text:"combined_results=kg_results+retrieved_results",why:"문자열과 list 타입이 충돌할 수 있습니다."},{text:"combined_results.append(retrieved_results)",why:"검색 결과 list 전체가 한 요소가 됩니다."},{text:"combined_results.extend(retrieved_results)",why:"정답입니다."},{text:"retrieved_results.clear()",why:"Web 근거를 삭제합니다."}]},
        {id:"mcp-m3",source_question_id:"mcp-q15",topic:"MCP Tool",prompt:"MCP server의 도구를 FunctionAgent에 전달 가능한 목록으로 가져오는 호출은?",answer_index:0,explanation:"McpToolSpec의 비동기 tool 변환 메서드를 사용합니다.",choices:[{text:"await mcp_tool.to_tool_list_async()",why:"정답입니다."},{text:"mcp_client.query_tools()",why:"노트북 API가 아닙니다."},{text:"await mcp_tool.fetch_resources()",why:"resource 목록이며 tool 목록이 아닙니다."},{text:"FunctionAgent.fetch()",why:"agent 생성 전 단계입니다."},{text:"BasicMCPClient.tools",why:"직접 속성 접근이 아닙니다."}]},
        {id:"mcp-m4",source_question_id:"mcp-q20",topic:"Agent Context",prompt:"공유 workflow 상태를 유지하며 agent를 실행하는 코드는?",answer_index:4,explanation:"run에 기존 Context를 ctx로 전달합니다.",choices:[{text:"agent.run(question)",why:"실행은 되더라도 명시한 공유 context가 빠집니다."},{text:"Context.run(question)",why:"Context가 agent 실행기가 아닙니다."},{text:"handler.run(agent)",why:"handler는 실행 결과입니다."},{text:"agent.query(ctx)",why:"해당 호출 흐름이 아닙니다."},{text:"self.agent.run(question, ctx=self.agent_context)",why:"정답입니다."}]},
        {id:"mcp-m5",source_question_id:"mcp-q27",topic:"Async RAG",prompt:"MCP retrieval 결과를 받은 뒤에만 생성하도록 보장하는 표현은?",answer_index:2,explanation:"비동기 retrieve coroutine을 await해야 실제 결과 문자열을 얻습니다.",choices:[{text:"self.retrieve(...)만 호출",why:"coroutine 객체만 얻습니다."},{text:"yield self.retrieve(...) ",why:"generator 흐름이 아닙니다."},{text:"await self.retrieve(query, query_time, search_results, topk)",why:"정답입니다."},{text:"sleep(1)",why:"완료를 보장하지 않습니다."},{text:"self.retrieve(...).result",why:"이 coroutine 사용법이 아닙니다."}]}
      ]
    }]
  };
})();
